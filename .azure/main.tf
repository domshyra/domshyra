...
resource "godaddy-dns_record" "a_record" {
  ...
  type   = "A"
  name   = "@"
  data   = azurerm_windows_web_app.repository_name[each.value].outbound_ip_address_list[length(azurerm_windows_web_app.repository_name[each.value].outbound_ip_address_list) - 1] # Use the last outbound IP address from the web app
